...
export const Shockwave: React.FC<ShockwaveProps> = ({
  ...
  return (
    ...
    >
      ...
      >
        {/* Inner flash */}
        <circle cx={centerX} cy={centerY} r={flashRadius} fill={`${color}30`} opacity={flashOpacity} />

        {/* Main ring */}
        <circle cx={centerX} cy={centerY} r={radius} fill="none" stroke={color} strokeWidth={strokeWidth} opacity={opacity} />

        {/* Secondary thinner ring */}
        {f > 3 && (
          <circle cx={centerX} cy={centerY} r={Math.max(0, radius - 30)} fill="none" stroke={color} strokeWidth={1.5} opacity={opacity * 0.4} />
        )}
        ...
        {Array.from({ length: 12 }).map((_, i) => {
          ...
          return (
            <line
              ...
              x2={centerX + Math.cos(angle) * rayLen}
              y2={centerY + Math.sin(angle) * rayLen}
              stroke={color} strokeWidth={1.5} opacity={rayOp}
            />
          );
        })}
        ...
        {DEBRIS.map((d, i) => {
          ...
          const dy = centerY + Math.sin(d.angle) * dist;
...
/** Multiple shockwave rings with staggered delays */
export const ShockwaveMulti: React.FC<{
  ...
  centerX?: number;
  centerY?: number;
}> = ({ color = "#00d4ff", count = 3, stagger = 8, maxRadius = 900, centerX, centerY }) => {
  return (
    <>
