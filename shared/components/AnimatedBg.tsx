 ...
 *
 * Production-level background with:
 ...
 *
 * Fully generic — works with any color palette.
 */
...
export interface AnimatedBgProps {
  ...
  bgColor?: string;
  /** Slightly lighter background for gradient */
  bgLightColor?: string;
...
export const AnimatedBg: React.FC<AnimatedBgProps> = ({
  ...
  const bgLight = bgLightColor ?? darkenHex(bgColor, -20);
  ...
  const nebulaX = interpolate(Math.sin(t * 0.3), [-1, 1], [20, 80]);
  const nebulaY = interpolate(Math.cos(t * 0.25 + 0.5), [-1, 1], [25, 65]);
    ...
    >
      ...
      <div
        style={{
          position: "absolute",
          inset: -20,
          background: `linear-gradient(160deg, ${bgColor} 0%, ${darkenHex(bgColor, 10)} 30%, ${bgLight} 60%, ${bgColor} 100%)`,
        }}
      />
